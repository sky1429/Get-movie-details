    ...
    with open(param, "r", encoding="utf-8") as file :
        ...
        for row in reader:
            ...
            for movie_people in movie_peoplelist:
                ...
                peopledict.update({movie_people_name[0] if movie_people_name else "":movie_people_role[0] if movie_people_role else ""})
            #保存获取到的数据
...
def sava_movie_info(param, movie_info):
    ...
    with open(param, "w", encoding="utf-8", newline="") as file:
        ...
        writer = csv.DictWriter(file, ["电影名称", "年份", "上映时间", "电影类型", "电影时长", "评分", "语言", "制作组", "简介"])
